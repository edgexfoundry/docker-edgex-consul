...
pipeline {
    ...
    stages {
                    ...
                    stages {
                        stage('Docker Build') {
                            steps {
                                ...
                                script {
                                    image_amd64 = docker.build('docker-edgex-consul', '-f Dockerfile .')
                                }
                            }
                        }
                    ...
                    stages {
                        stage('Docker Build') {
                            steps {
                                ...
                                script {
                                    image_arm64 = docker.build('docker-edgex-consul-arm64', '-f Dockerfile .')
                                }
                            }
                        }
        ...
        stage('Clair Image Scan') {
            when { expression { edgex.isReleaseStream() } }
            steps {
                edgeXClair("${env.DOCKER_REGISTRY}:10004/docker-edgex-consul:1.1.0")
                edgeXClair("${env.DOCKER_REGISTRY}:10004/docker-edgex-consul-arm64:1.1.0")
            }
        }
    }
